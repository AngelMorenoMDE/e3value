...
public class OrJoinCreateCommand extends EditElementCommand {
	...
	protected CommandResult doExecuteWithResult(IProgressMonitor monitor,
		...
		OrJoin orJoin = E3valueFactory.eINSTANCE.createOrJoin();
		...
		orJoin.getHasInputPorts().add(inputport1);
		orJoin.getHasInputPorts().add(inputport2);
		orJoin.setHasOutputPort(outputport2);
		
		if (getElementToEdit() instanceof ValueActivity) {
			ValueActivity owner = (ValueActivity) getElementToEdit();
			owner.getHasDependenceElements().add(orJoin);
		} else {
			BusinessActor owner = (BusinessActor) getElementToEdit();
			owner.getHasDependenceElements().add(orJoin);
		}

		doConfigure(orJoin, monitor, info);

		((CreateElementRequest) getRequest()).setNewElement(orJoin);
		return CommandResult.newOKCommandResult(orJoin);
	}
